...
Based on train_v17: eval opponents changed from random to v6.
...
def train(num_iter=200, batch=50, lr=0.0003, version=None, terminal_win=5.0):
    ...
    print(f"  Training: 100% self-play | Mixed eval every {EVAL_EVERY} iters ({EVAL_GAMES} games: 50% self-best, 25% v3, 25% v6)")
    ...
    # Load v3 and v6 as fixed eval opponents
    import importlib.util as _ilu
    _base = os.path.dirname(os.path.abspath(__file__))

    _v3_path = os.path.join(_base, "agent_submit_v3.py")
    _v3_mod = None
    if os.path.exists(_v3_path):
        _spec = _ilu.spec_from_file_location("agent_submit_v3", _v3_path)
        _v3_mod = _ilu.module_from_spec(_spec)
        _spec.loader.exec_module(_v3_mod)
        print(f"  Loaded v3 eval opponent from {_v3_path}")
    else:
        print(f"  WARNING: {_v3_path} not found, v3 eval disabled")

    _v6_path = os.path.join(_base, "agent_submit_v6.py")
    _v6_mod = None
    ...
    for it in range(num_iter):
        ...
        # Periodic mixed eval: 50% self-best, 25% v3, 25% v6
        eval_str = ""
        if (it + 1) % EVAL_EVERY == 0:
            n_self = EVAL_GAMES // 2
            n_v3 = EVAL_GAMES // 4
            n_v6 = EVAL_GAMES - n_self - n_v3
            ev_wins = 0
            ei = 0
            ...
            if _v3_mod is not None:
                for _ in range(n_v3):
                    ...
                    STATE.update({"turn": 0, "nodes": set(), "last_factory_pos": None,
                                  "factory_stuck": 0, "walls": {}})
                    if hasattr(_v3_mod, 'STATE'):
                        _v3_mod.STATE.update({"turn": 0, "nodes": set(), "last_factory_pos": None,
                                               "factory_stuck": 0, "walls": {}})

                    env = make("crawl", configuration={"randomSeed": seed}, debug=True)
                    env.run([_make_greedy_agent(model), _v3_mod.agent])
                    final = env.steps[-1]
                    er0, er1 = final[0].reward, final[1].reward
                    if er0 > er1: ev_wins += 1
                    ei += 1
