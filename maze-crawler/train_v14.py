...
REINFORCE (no value baseline, no GAE, no clipped surrogate).
"""
...
def _compute_step_reward(obs, config, my_player, prev_total_energy, prev_units, prev_gap):
    ...
    delta_units = (cur_units - prev_units) * UNIT_WEIGHT
...
def train(num_iter=200, batch=50, lr=0.0003, version=None):
    ...
    print(f"=== Train v{version} (All-unit NN, REINFORCE vs random) ===")
    print(f"  Input: {INPUT_SIZE} | Actions: {NUM_ACTIONS}")
    print(f"  Reward: delta_e/1000 + delta_units*{UNIT_WEIGHT} + delta_gap*0.5 + 0.01 + terminal +5/-1")
