"""PPO training — v19: self-play vs v49 best, mixed eval (50% self-best + 25% v6 + 25% v49 best).

Based on train_v17.py with these changes:
- Training opponent: self-play against v49 best (instead of random in v17's self-play vs random)
  Wait, actually re-reading: v17 training is 100% self-play. The "opponent=random" was only for
  the eval's 25% random portion. So the training loop itself is unchanged (100% self-play).
  
Actually: The user's request is specifically about the eval opponents:
- v17 eval: 50% self-best + 25% v6 + 25% random
- v19 eval: 50% self-best + 25% v6 + 25% v49 best

The training is still 100% self-play, unchanged.
"""
import sys, os, random, time, importlib.util as _ilu
...
def train(num_iter=2000, batch=100, lr=0.0003, version=None, terminal_win=5.0):
    ...
    log_path = f"train_v19_v{version}.log"

    print(f"  train_v19.py — mixed eval: 50% self-best + 25% v6 + 25% v49 best")
    ...
    with open(log_path, "w") as log_f:
        log_f.write(f"train_v19.py | eval=50%self+25%v6+25%v49 | iters={num_iter} | batch={batch}\n\n")
        ...
        for it in range(num_iter):
            ...
            # ── Periodic mixed eval: 50% self-best, 25% v6, 25% v49 best ──
            eval_str = ""
            if (it + 1) % EVAL_EVERY == 0:
                n_self = EVAL_GAMES // 2
                n_v6 = EVAL_GAMES // 4
                n_v49 = EVAL_GAMES - n_self - n_v6
                ev_wins = 0
                ei = 0
                ...
                # 25% vs v49 best (replaces v17's 25% random)
                for _ in range(n_v49):
                    ...
                    def _make_v49_eval(m_train, m_v49, ps):
                        def _agent(obs, config):
                            mp = obs.player
                            saved = _swap_state(ps, mp)
                            try:
                                ...
                                for uid2, d2 in obs.robots.items():
                                    ...
                                    m = torch.FloatTensor(msk).unsqueeze(0)
                                    with torch.no_grad():
                                        probs, _ = (m_train if mp == 0 else m_v49)(s, m)
                                        ai = torch.argmax(probs).item()
                                    execute_action(uid2, d2, ai, actions, reserved)
                                return actions
                            finally:
                    ...
                    env.run([_make_v49_eval(model, v49_model, _pstates),
                             _make_v49_eval(model, v49_model, _pstates)])
                    final = env.steps[-1]
                    er0, er1 = final[0].reward, final[1].reward
                    if er0 > er1: ev_wins += 1
                    ei += 1
