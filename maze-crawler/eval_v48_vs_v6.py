"""Head-to-head: v48 (all-unit NN) vs v6 (factory-only NN + rules)."""
import sys, os
...
from agent_v1 import update_state, STATE as S17

model48 = ActorCritic()
model48.load_state_dict(torch.load('nn_weights_v48.pt'))
model48.eval()
print("Loaded v48 best weights", flush=True)

import importlib.util
spec = importlib.util.spec_from_file_location('agent_submit_v6', 'agent_submit_v6.py')
v6_mod = importlib.util.module_from_spec(spec)
spec.loader.exec_module(v6_mod)
print("Loaded v6 submission agent", flush=True)
...
for i in range(NUM_GAMES):
    ...
    S17.update({"turn": 0, "nodes": set(), "last_factory_pos": None,
                "factory_stuck": 0, "walls": {}})
    if hasattr(v6_mod, 'STATE'):
        v6_mod.STATE.update({"turn": 0, "nodes": set(), "last_factory_pos": None,
                              "factory_stuck": 0, "walls": {}})

    env = make("crawl", configuration={"randomSeed": seed}, debug=True)

    def make_v48_agent():
        def v48_agent(obs, config):
    ...
    env.run([make_v48_agent(), v6_mod.agent])
    final = env.steps[-1]
...
print(f"\n=== v48 vs v6: {wins}W-{losses}L-{draws}D ({wins/NUM_GAMES*100:.1f}%) ===", flush=True)
