"""PPO training — v21: mixed eval (20% self-best + 40% v50 best + 20% v49 best + 20% agent_v1).

Changes from v20:
- Eval opponents: 20% self-best + 40% v50 best + 20% v49 best + 20% agent_v1
- Heavier weight on v50 best (strongest known model) and includes rule-based agent_v1
...
def train(num_iter=2000, batch=100, lr=0.0003, version=None, terminal_win=5.0):
    ...
    log_path = f"train_v21_v{version}.log"

    print(f"  train_v21.py — mixed eval: 20% self-best + 40% v50 best + 20% v49 best + 20% agent_v1")
    ...
    # Import agent_v1 as eval opponent
    agent_v1_fn = None
    try:
        agent_v1_mod = __import__(AGENT_V1_MODULE)
        for attr_name in ["agent_v1", "agent", "my_agent", "Agent"]:
            fn = getattr(agent_v1_mod, attr_name, None)
            if callable(fn):
                agent_v1_fn = fn
                break
        if agent_v1_fn is None:
            print(f"  WARNING: No agent function found in {AGENT_V1_MODULE}.py, skipping agent_v1 eval")
        else:
            print(f"  Loaded agent_v1 eval opponent from {AGENT_V1_MODULE}.py ({agent_v1_fn.__name__})")
    except ImportError as e:
        print(f"  WARNING: Could not import {AGENT_V1_MODULE}: {e}, skipping agent_v1 eval")
    ...
    with open(log_path, "w") as log_f:
        log_f.write(f"train_v21.py | eval=20%self+40%v50+20%v49+20%agent_v1 | iters={num_iter} | batch={batch}\n\n")
        ...
        for it in range(num_iter):
            ...
            # ── Periodic mixed eval: 20% self-best, 40% v50 best, 20% v49 best, 20% agent_v1 ──
            eval_str = ""
            if (it + 1) % EVAL_EVERY == 0:
                # 20% self-best + 40% v50 + 20% v49 + 20% agent_v1
                n_self = int(EVAL_GAMES * 0.2)
                n_v50 = int(EVAL_GAMES * 0.4)
                n_v49 = int(EVAL_GAMES * 0.2)
                n_a1 = EVAL_GAMES - n_self - n_v50 - n_v49
                ev_wins = 0
                ei = 0
                ...
                # 20% vs agent_v1 (with STATE isolation)
                if agent_v1_fn is not None:
                    for _ in range(n_a1):
                        seed = ei * 137 + 42
                        _pstates = {0: dict(_INITIAL_STATE), 1: dict(_INITIAL_STATE)}
                        _pstates[0]["nodes"] = set()
                        ...
                        def _make_nn_agent_isolated(mdl, ps):
                            """NN agent with per-player STATE isolation."""
                            def _agent(obs, config):
                                mp = obs.player
                                saved = _swap_state(ps, mp)
                                try:
                        ...
                        def _make_a1_agent_isolated(a1_fn, ps):
                            """agent_v1 wrapper with STATE isolation."""
                            def _agent(obs, config):
                                mp = obs.player
                                saved = _swap_state(ps, mp)
                                try:
                                    return a1_fn(obs, config)
                                finally:
                                    _restore_state(ps, mp, saved)
                            return _agent

                        env.run([_make_nn_agent_isolated(model, _pstates),
                                 _make_a1_agent_isolated(agent_v1_fn, _pstates)])
                        final = env.steps[-1]
                        if final[0].reward > final[1].reward: ev_wins += 1
                        ei += 1
...
def _run_eval(model, opponent_fn, num_games, label):
    ...
    for i in range(num_games):
        ...
        def _make_opponent_isolated(opp_fn, ps):
            """Opponent wrapper with STATE isolation (for agent_v1 / rule-based agents)."""
            def _agent(obs, config):
                mp = obs.player
                saved = _swap_state(ps, mp)
                try:
                    return opp_fn(obs, config)
                finally:
                    _restore_state(ps, mp, saved)
            return _agent
        ...
        if isinstance(opponent_fn, str):
            # Built-in opponent like "random"
            env.run([nn_agent, opponent_fn])
        else:
            env.run([nn_agent, _make_opponent_isolated(opponent_fn, _pstates)])
        final = env.steps[-1]
        r0, r1 = final[0].reward, final[1].reward
        if r0 > r1: wins += 1
        elif r0 < r1: losses += 1
...
def final_evaluate(model, version):
    ...
    # Load agent_v1 for final evaluation
    agent_v1_fn = None
    try:
        agent_v1_mod = __import__(AGENT_V1_MODULE)
        for attr_name in ["agent", "agent_v1", "my_agent", "Agent"]:
            fn = getattr(agent_v1_mod, attr_name, None)
            if callable(fn):
                agent_v1_fn = fn
                break
    ...
    if agent_v1_fn is not None:
        _run_eval(model, agent_v1_fn, 500, "agent_v1")
