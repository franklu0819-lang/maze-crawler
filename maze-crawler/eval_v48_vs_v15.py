"""Head-to-head: v48 (all-unit NN) vs v15 (factory-only NN + rules)."""
import sys, os
...
spec = importlib.util.spec_from_file_location('agent_submit_v15', 'agent_submit_v15.py')
v15_mod = importlib.util.module_from_spec(spec)
spec.loader.exec_module(v15_mod)
print("Loaded v15 submission agent", flush=True)
...
for i in range(NUM_GAMES):
    ...
    if hasattr(v15_mod, 'STATE'):
        v15_mod.STATE.update({"turn": 0, "nodes": set(), "last_factory_pos": None,
                              "factory_stuck": 0, "walls": {}})
    ...
    env.run([make_v48_agent(), v15_mod.agent])
    final = env.steps[-1]
...
print(f"\n=== v48 vs v15: {wins}W-{losses}L-{draws}D ({wins/NUM_GAMES*100:.1f}%) ===", flush=True)
