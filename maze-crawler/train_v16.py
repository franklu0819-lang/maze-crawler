"""REINFORCE training with NN controlling ALL units + per-unit behavioral shaping.

Same reward as train_v15.py, but uses REINFORCE (Monte Carlo returns) instead of PPO.
No value baseline, no GAE, no clipped surrogate.
"""
...
def _compute_step_reward(obs, config, my_player, prev_total_energy, prev_units, prev_gap):
    ...
    delta_units = (cur_units - prev_units) * 0.1
...
def run_game(model, seed, explore=True):
    _reset_state()
    env = make("crawl", configuration={"randomSeed": seed}, debug=True)
    unit_trajs = {}  # uid -> [(feat, ai, mask, log_p, step_r)]
        ...
        for unit_type in [TYPE_SCOUT, TYPE_WORKER, TYPE_MINER, TYPE_FACTORY]:
            for uid2, d2 in obs.robots.items():
                ...
                unit_trajs[uid2].append(
                    (feat.copy(), ai_item, msk.copy(), log_p_item, step_r))
...
def compute_returns(traj, terminal_reward, gamma=GAMMA):
    ...
    rewards = [traj[i][4] for i in range(T)]
...
def reinforce_update(model, optimizer, all_feat, all_act, all_mask, all_ret):
    ...
    policy_loss = -(log_probs * returns).mean() - ENTROPY_COEF * entropy
...
def train(num_iter=200, batch=50, lr=0.0003, version=None):
    ...
    print(f"=== Train v{version} (All-unit NN, REINFORCE vs random, v10-based reward) ===")
    print(f"  Input: {INPUT_SIZE} | Actions: {NUM_ACTIONS}")
    print(f"  Team: delta_e/1000 + delta_gap*1.0 + delta_units*0.1 + survival*0.01")
    ...
    for it in range(num_iter):
        ...
        for _ in range(batch):
            ...
            unit_trajs, r0, r1 = run_game(model, seed, explore=True)
            ...
            for uid, traj in unit_trajs.items():
                rets = compute_returns(traj, terminal_r)
                for i, (feat, ai, msk, lp, sr) in enumerate(traj):
        ...
        pl, el = reinforce_update(model, optimizer, all_feat, all_act,
                                   all_mask, all_ret)

        wr = wins / batch * 100
        elapsed = time.time() - t0
        print(f"[{it+1:3d}/{num_iter}] WR={wr:5.1f}% p_loss={pl:.4f} "
              f"ent={el:.3f} n={len(all_feat)} t={elapsed:.0f}s")
...
def evaluate_vs_random(model, num_games=500):
    ...
    for i in range(num_games):
        seed = i * 137 + 42
        _, r0, r1 = run_game(model, seed, explore=False)
        if r0 > r1: wins += 1
        elif r0 < r1: losses += 1
        else: draws += 1
